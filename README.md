...
```bash
getletter$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
getletter$ sam local start-api
getletter$ curl http://localhost:3000/
```
...
```bash
getletter$ sam logs -n HelloWorldFunction --stack-name getletter --tail
```
...
```bash
sam delete --stack-name getletter
```
